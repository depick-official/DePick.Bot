...
export default function CustomArenaDashboardPage() {
  ...
  const [dashboardData, setDashboard] =
    useState<CustomArenaDashboardResponse | null>(null);
  const dashboard =
    dashboardData?.community.id === selectedCommunityId ? dashboardData : null;
  ...
  const [selectedMarket, setSelectedMarket] =
    useState<CustomArenaMarket | null>(null);
    ...
    customArenaApi
      ...
      .then((response) => {
        ...
        setSelectedCommunityId((current) => {
          if (
            current &&
            nextCommunities.some((community) => community.id === current)
          ) {
            return current;
          }
          if (
            launch.communityId &&
            nextCommunities.some(
              (community) => community.id === launch.communityId,
            )
          ) {
      ...
      .catch((err) => {
        if (!cancelled)
          setError(getApiErrorMessage(err, "Could not load your communities."));
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    setSelectedChannelKey((current) => {
      if (
        current &&
        selectedCommunity.channels.some(
          (channel) => scopeKey(channel) === current,
        )
      ) {
        return current;
      }
      const launchChannel =
        launch.scope &&
        selectedCommunity.channels.find(
          (channel) => scopeKey(channel) === scopeKey(launch.scope!),
        );
      if (launchChannel) return scopeKey(launchChannel);
      return selectedCommunity.channels.length === 1
        ? scopeKey(selectedCommunity.channels[0])
        : null;
    });
  ...
  useEffect(() => {
    ...
    customArenaApi
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(
            getApiErrorMessage(err, "Could not load this community dashboard."),
          );
        }
      })
      .finally(() => {
  ...
  const canCreate = Boolean(
    selectedCommunityId && selectedChannel && hasCapacity,
  );
  ...
  return (
    <div className="container custom-arena-dashboard">
      ...
      {isLoadingCommunities ? (
        ...
        <div className="empty">
          <p>No moderator communities are available.</p>
        </div>
      ) : (
        <>
          <section className="custom-arena-dashboard-panel custom-arena-dashboard-context">
            ...
            <div className="custom-arena-dashboard-selector">
              {selectedCommunity?.channels.length === 1 ? (
                <>
                  <span>Create in channel</span>
                  <strong>{channelLabel(selectedCommunity.channels[0])}</strong>
                </>
              ) : (
                <>
                  <label htmlFor="custom-arena-channel">
                    Create in channel
                  </label>
                  <select
                    ...
                    onChange={(event) =>
                      setSelectedChannelKey(event.target.value || null)
                    }
                    disabled={!selectedCommunity?.channels.length}
                  >
                    {!selectedChannelKey && (
                      <option value="">
                        {selectedCommunity?.channels.length
            ...
            {launch.scope &&
              selectedChannel &&
              scopeKey(launch.scope) === scopeKey(selectedChannel) && (
                <p className="custom-arena-dashboard-context-note">
                  Launch channel selected from{" "}
                  {launch.scope.scopeProvider.toLowerCase()}.
                </p>
              )}
            {dashboard && !hasCapacity && (
              <div
                className="custom-arena-dashboard-capacity-notice"
                role="status"
              >
                <strong>PICK capacity required</strong>
                <p>
                  Ask a dePICK admin to activate capacity before creating a
                  market.
                </p>
              </div>
            )}
          ...
          {isLoadingDashboard && !dashboard ? (
            ...
            <>
              <section
                className="custom-arena-dashboard-panel"
              ...
              >
                <div className="custom-arena-dashboard-panel-heading">
                  <div>
                    ...
                    <p>
                      {dashboard.community.mode || "SOCIAL"} ·{" "}
                      {hasCapacity
                        ? dashboard.community.status || "ACTIVE"
                        : "CAPACITY REQUIRED"}
                    </p>
                  </div>
                ...
                <div className="custom-arena-dashboard-stat-grid">
                  <div className="custom-arena-dashboard-stat">
                    <span>Shared community capacity</span>
                    <strong>
                      {formatPick(dashboard.capacity.availablePick)}
                    </strong>
                    <small>of {formatPick(dashboard.capacity.totalPick)}</small>
                  </div>
                  <div className="custom-arena-dashboard-stat">
                    <span>Committed reserve</span>
                    <strong>
                      {formatPick(dashboard.capacity.reservedPick)}
                    </strong>
                  </div>
                  <div className="custom-arena-dashboard-stat">
              ...
              <section
                className="custom-arena-dashboard-panel"
                aria-label="Market status filters"
              >
                <div className="custom-arena-dashboard-panel-heading">
                  <h2>Market status</h2>
                  ...
                  {statusFilters.map((filter) => (
                    ...
                    >
                      ...
                      <strong>
                        {filter.value === "all"
                          ? totalCount(counts)
                          : countForStatus(counts, filter.value)}
                      </strong>
                    </button>
                  ))}
                </div>
              ...
              <section
                className="custom-arena-dashboard-markets"
                aria-label="Community markets"
                aria-busy={isLoadingDashboard}
              >
                {error && (
                  <div className="custom-arena-dashboard-error" role="alert">
                ...
                {isLoadingDashboard && (
                  <div
                    className="custom-arena-dashboard-markets-loading"
                    role="status"
                  >
                    Loading markets...
                  </div>
                ...
                {dashboard.markets.length === 0 ? (
                  <div className="empty">
                    <p>No markets match this status.</p>
                  </div>
                ) : (
...
function useLaunchContext(searchParams: URLSearchParams) {
  return useMemo(() => {
    ...
    const externalId =
      searchParams.get("scope_external_id") ||
    ...
    const displayName =
      searchParams.get("scope_display_name") ||
      searchParams.get("displayName") ||
      searchParams.get("channel_name") ||
      searchParams.get("channelName") ||
      undefined;
    const communityId =
      searchParams.get("community_id") || searchParams.get("communityId");
    const scope =
      provider && externalId
...
function channelLabel(channel: CustomArenaScope) {
  const platform =
    channel.scopeProvider === "TELEGRAM" ? "Telegram" : "Discord";
  return `${platform} · ${channel.displayName || channel.scopeExternalId}`;
}

function communityLabel(community: CustomArenaCommunity) {
  const platform =
    community.channels[0]?.scopeProvider === "TELEGRAM"
      ? "Telegram"
      : community.channels[0]?.scopeProvider === "DISCORD"
        ? "Discord"
        : "Unknown";
  return `${community.displayName} · ${platform}`;
}
